...
<dict>
    ...
    <array>
        ...
        <string>-c</string>
        <string>/bin/launchctl setenv SSH_ASKPASS ${homebrew_prefix}/bin/ssh-askpass; /bin/launchctl setenv SSH_ASKPASS_REQUIRE force; /bin/launchctl setenv SSH_SK_PROVIDER /usr/local/lib/libsk-libfido2.dylib; killall ssh-agent; /usr/bin/ssh-add -l; /bin/launchctl unsetenv SSH_ASKPASS_REQUIRE</string>
    </array>
    <key>RunAtLoad</key>
    <true/>
</dict>
